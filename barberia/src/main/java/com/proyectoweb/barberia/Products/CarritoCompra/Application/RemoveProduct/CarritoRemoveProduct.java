package com.proyectoweb.barberia.Products.CarritoCompra.Application.RemoveProduct;
...
public class CarritoRemoveProduct {

    private CarritoCompraRepository repository;

    public CarritoRemoveProduct(CarritoCompraRepository repository){
    ...
    public void execute(String carritoCompraId, String productId, String name, Integer quantity, Double price){
        ...
        CarritoCompra compraFinal = compra.get();
        compraFinal.removeProduct(new ProductDetails(productId, name, quantity, price));
        repository.update(compraFinal);
    }
}
